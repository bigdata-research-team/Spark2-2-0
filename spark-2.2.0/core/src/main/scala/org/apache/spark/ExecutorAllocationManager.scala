...
private[spark] class ExecutorAllocationManager(
   ...
   */
  def start(): Unit = {
    // 1、向事件总线添加ExecutorAllocationListener
    listenerBus.addListener(listener)

    // 2、创建定时调度任务scheduTask，调用schedule方法
    ...
    //3、将scheduleTask提交给executor，以固定时间间隔100进行调度
    executor.scheduleWithFixedDelay(scheduleTask, 0, intervalMillis, TimeUnit.MILLISECONDS)
    //4、请求所有Executor,3条参数最大值，本地设置Task数，Host和此节点上的Task的数量之间的映射
  ...
  private def updateAndSyncNumExecutorsTarget(now: Long): Int = synchronized {
    // 获取实际需要的最大的Executor数量
    val maxNeeded = maxNumExecutorsNeeded

    // 判断ExecutorAllocationManager是否还在初始化，初始化返回0
    if (initializing) {
      ...
      /**
        * 如果maxNeeded大于等于numExecutorsTarget且当前时间大于上次添加Executor的时间，那么首先调用addExecutors方法（
        * 此方法也是利用ExecutorAllocationClient的requestTotalExecutors方法）通知集群管理器添加额外的Executor，然后更新添加Executor的时间，
        * 最后返回添加的Executor数量。
        */
