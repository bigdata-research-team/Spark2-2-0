...
private[spark] class ActiveJob(
    ...
    val properties: Properties) { // 包含了当前Job调度、Job group、描述等属性的Properties

  /**
   * Number of partitions we need to compute for this job. Note that result stages may not need
   * to compute all partitions in their target RDD, for actions like first() and lookup().
   */
  val numPartitions = finalStage match { // 当前job的分区数
    case r: ResultStage => r.partitions.length // 若finalStage为ResultStage，则此属性等于ResultStage的partitions属性的长度
    case m: ShuffleMapStage => m.rdd.partitions.length // 若finalStage为ShuffleMapStage，则此属性等于ShuffleMapStage的Rdd的partitions属性的长度
  }

  /** Which partitions of the stage have finished */
  val finished = Array.fill[Boolean](numPartitions)(false) // Boolean类型数组，每个数组索引代表一个分区的任务是否执行完成
