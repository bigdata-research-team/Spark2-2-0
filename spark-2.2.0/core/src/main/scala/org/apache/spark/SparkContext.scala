...
class SparkContext(config: SparkConf) extends Logging {
  ...
  private var _conf: SparkConf = _   //SparkContext的配置，通过调用SparkConf的clone方法的克隆体，在SparkContext初始化过程中会对conf中的配置信息做校验
  private var _eventLogDir: Option[URI] = None  // 用户设置的jar文件
  private var _eventLogCodec: Option[String] = None  // 事件日志压缩算法，当Spark.eventLog.enabled属性与spark.eventLog.compress属性皆为true时启用，默认为lz4压缩算法，也可以通过spark.io.compression.codec属性指定
   ...
   */
  def isStopped: Boolean = stopped.get()ƒ
  ...
  try {
    ...
    // 使用addJar和addFile将文件添加到Driver的RPC环境中
    if (jars != null) {
      jars.foreach(addJar)
    }
    ...
    // 1、判断是否需要启用ExecutorAllocationManager
    ...
    _executorAllocationManager =
      if (dynamicAllocationEnabled) {
        // 2、看SchedulerBackend的实现类是否实现了特质ExecutorAllocationClient,实现了才会创建ExecutorAllocationManager
    ...
    // 3、启动ExecutorAllocationManager
    _executorAllocationManager.foreach(_.start())

    // 判断是否启用ContextCleaner
    _cleaner =
  ...
  def addFile(path: String, recursive: Boolean): Unit = {
    ...
    val key = if (!isLocal && scheme == "file") {
      // 将文件添加到Driver本地RpcEnv环境中，并添加时间戳信息
  ...
  private def setupAndStartListenerBus(): Unit = {
    ...
    try {
      // 获取用户自定义的SparkListener的类名，可通过"，"分隔多个自定义SparkListener
      val listenerClassNames: Seq[String] =
        conf.get("spark.extraListeners", "").split(',').map(_.trim).filter(_ != "")
      // 通过反射生成每一个自定义SparkListener实例，并添加到事件总线的监听器列表中
