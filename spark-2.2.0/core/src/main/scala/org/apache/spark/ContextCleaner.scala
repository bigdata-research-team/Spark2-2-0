...
private[spark] class ContextCleaner(sc: SparkContext) extends Logging {
   ...
   * resolved.
   */
    // 清理shuffle数据是否是阻塞式的，清理shuffle数据包括清理MapOutputTracker中指定的ShuffleId对应的map任务状态和
    // ShuffleManager中注册的shuffleId对应的Shuffle元数据
  ...
  def start(): Unit = {
    ...
    // 2、给periodicGCService设置以periodicGCInterval作为时间间隔定时进行GC的操作任务
