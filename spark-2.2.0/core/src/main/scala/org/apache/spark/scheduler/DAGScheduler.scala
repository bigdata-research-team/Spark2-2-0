...
class DAGScheduler(
  ...
  private[spark] val metricsSource: DAGSchedulerSource = new DAGSchedulerSource(this) // 有关DAGScheduler的度量源
   ...
   */
    // 缓存每个RDD的所有分区的位置信息，根据数据类型可知，每个RDD的分区按照分区号作为索引存储到IndexedSeq。
    // 由于RDD的每个分区的Block可能存在多个节点的BlockManager上，RDD每个分区的位置信息为TaskLocation的序列
  ...
  // 当检测到一个节点出现故障时，会将执行失败的Executor和MapOutputTracker当前的纪元添加到failedEpoch，此外，还使用failedEpoch忽略迷失的ShuffleMapTask的结果
  ...
  //用于获取RDD  各个分区TaskLocation序列
  private[scheduler]
  def getCacheLocs(rdd: RDD[_]): IndexedSeq[Seq[TaskLocation]] = cacheLocs.synchronized {
    // Note: this doesn't use `getOrElse()` because this method is called O(num tasks) times
    if (!cacheLocs.contains(rdd.id)) { // 判断是否包含
      // Note: if the storage level is NONE, we don't need to get locations from block manager.
      // 若RDD的存储级别为None
      val locs: IndexedSeq[Seq[TaskLocation]] = if (rdd.getStorageLevel == StorageLevel.NONE) {
        IndexedSeq.fill(rdd.partitions.length)(Nil) // 构造一个空序列
      } else {
        // 若存储级别不为None
        val blockIds =
          rdd.partitions.indices.map(index => RDDBlockId(rdd.id, index)).toArray[BlockId]
        blockManagerMaster.getLocations(blockIds).map { bms => // 获取每个RDDBlockId所存储的位置信息
          bms.map(bm => TaskLocation(bm.host, bm.executorId)) // 封装为TaskLocation
        }
      }
      // 将RDDiD与序列的映射关系放入cacheLocs
  ...
  private def createResultStage(
    ...
    // 获取之前的所有父Stage的列表，此列表包含
    // 1、当前RDD的直接或间接依赖是ShuffleDependency且已经注册过的Stage
    // 2、当前RDD的直接或间接依赖是ShuffleDependency且没有注册过Stage的，对于这种ShuffleDependency根据其中的RDD找到它的直接或间接
    // 的依赖是ShuffleDependency且没有注册过的Stage的所有ShuffleDependency，为他们创建并注册Stage
    // 3、当前RDD的直接或间接依赖是ShuffleDependency且没有注册过Stage的，为ShuffleDependency创建并注册Stage
    val parents = getOrCreateParentStages(rdd, jobId)
    val id = nextStageId.getAndIncrement() // 创建新（自己）Stage的ID
    val stage = new ResultStage(id, rdd, func, partitions, parents, jobId, callSite) // 创建Stage，参数中要有parents（所有的父Stage）
    stageIdToStage(id) = stage // 将ResultStage注册到StageIdToStage中
    updateJobIdStageIdMaps(jobId, stage) // 更新Jobid与ResultStage及其所有祖先的映射关系
    stage // 返回resultStage
  }
   ...
   */
  private def getOrCreateParentStages(rdd: RDD[_], firstJobId: Int): List[Stage] = { // 用于获取或创建给定RDD的所有父Stage，这些Stage将被分配给jobId对应的Job
    getShuffleDependencies(rdd).map { shuffleDep => // 获取RDD的所有shuffleDependency序列，逐个访问每个RDD及其依赖的非Shuffle的RDD，获取所有非shuffle的RDD的ShuffleDependency依赖
      getOrCreateShuffleMapStage(shuffleDep, firstJobId) // 获取或创建ShuffleMapStage
    }.toList
  }
  ...
  private[scheduler] def getShuffleDependencies(
    ...
    val parents = new HashSet[ShuffleDependency[_, _, _]] // 创建容器
    val visited = new HashSet[RDD[_]]// 创建容器
    val waitingForVisit = new Stack[RDD[_]]// 创建容器
    waitingForVisit.push(rdd) // 向容器中放入RDD
    while (waitingForVisit.nonEmpty) {
      val toVisit = waitingForVisit.pop() // 取出并删除放入的RDD,toVisit: RDD[]
      if (!visited(toVisit)) { // 判断容器中rdd是否存在，不存在则加入
        visited += toVisit
        toVisit.dependencies.foreach { // 获取容器中RDD的依赖关系一一匹配
          case shuffleDep: ShuffleDependency[_, _, _] => // 若为宽依赖则将加入parents
            parents += shuffleDep
          case dependency =>
            waitingForVisit.push(dependency.rdd) // 若为narrowDependency 则将rdd 加入容器，再次进行遍历
        }
      }
    }
    parents
  ...
  def submitJob[T, U](
    ...
    val maxPartitions = rdd.partitions.length // 获取当前Job的最大分区数，来进行下面的分区安全验证
    partitions.find(p => p >= maxPartitions || p < 0).foreach { p => // 安全监测，如果分区大于最大分区数或者小于0，则抛异常
    ...
    if (partitions.size == 0) { // 判断Job分区数
      // Return immediately if the job is running 0 tasks
      return new JobWaiter[U](this, jobId, 0, resultHandler) // 返回totalTasks为0的JobWaiter，在JobWaiter中JobPromise根据totalTasks设置为true
    }

    assert(partitions.size > 0) // 判断分区数，大于0则创建真正等待Job执行完成的JobWaiter
    val func2 = func.asInstanceOf[(TaskContext, Iterator[_]) => _]
    val waiter = new JobWaiter(this, jobId, partitions.size, resultHandler) // 创建JobWaiter监听器
    eventProcessLoop.post(JobSubmitted( // 事件池，放进去了一个事件
  ...
  def runJob[T, U](
    ...
    val start = System.nanoTime // 获取时间
    val waiter = submitJob(rdd, func, partitions, callSite, resultHandler, properties) // 异步提交，立即返回监听器类型
    ThreadUtils.awaitReady(waiter.completionFuture, Duration.Inf)
    waiter.completionFuture.value.get match { // 等待job任务处理完毕
      case scala.util.Success(_) => // 如果成功打印结果
        logInfo("Job %d finished: %s, took %f s".format
          (waiter.jobId, callSite.shortForm, (System.nanoTime - start) / 1e9))
      case scala.util.Failure(exception) =>  // 如果失败打印结果并抛异常
        logInfo("Job %d failed: %s, took %f s".format
          (waiter.jobId, callSite.shortForm, (System.nanoTime - start) / 1e9))
        // SPARK-8644: Include user stack trace in exceptions coming from DAGScheduler.
        val callerStackTrace = Thread.currentThread().getStackTrace.tail // 获取线程信息
        exception.setStackTrace(exception.getStackTrace ++ callerStackTrace) // 将信息抛异常
        throw exception
    }
  }
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    var finalStage: ResultStage = null // 先创建最后一个Stage的引用
    try {
      // New stage creation may throw an exception if, for example, jobs are run on a
      // HadoopRDD whose underlying HDFS files have been deleted.
      finalStage = createResultStage(finalRDD, func, partitions, jobId, callSite)// 先创建最后一个Stage，重点方法
    } catch { // 若发生异常，则需要主动调用JobWaiter的JobFailed方法
      case e: Exception =>
    ...
    //创建ActiveJob
    val job = new ActiveJob(jobId, finalStage, callSite, listener, properties) // 创建激活的job，收尾
    clearCacheLocs() //清空cacheLocs中缓存的各个RDD的所有分区位置信息
    ...
    val jobSubmissionTime = clock.getTimeMillis() // 生成Job提交的时间
    jobIdToActiveJob(jobId) = job // 将JobId与刚创建的ActiveJob之间的对应关系放入jobIdToActiveJob中
    activeJobs += job // 将刚创建的ActiveJob放入activeJob集合中
    finalStage.setActiveJob(job) // 使ResultStage的_activeJob属性持有刚创建的ActiveJob
    val stageIds = jobIdToStageIds(jobId).toArray // 获取当前Job的所有Stage对应的StageId
    val stageInfos = stageIds.flatMap(id => stageIdToStage.get(id).map(_.latestInfo)) // 获取当前Job的所有Stage对应的StageId的StageInfo
    listenerBus.post( // 向LiveListenerBus投递SparkListenerJobStart事件
      SparkListenerJobStart(job.jobId, jobSubmissionTime, stageInfos, properties))
    submitStage(finalStage) // 提交ResultStage
  }
...
private[scheduler] class DAGSchedulerEventProcessLoop(dagScheduler: DAGScheduler)
  ...
  private def doOnReceive(event: DAGSchedulerEvent): Unit = event match {
    case JobSubmitted(jobId, rdd, func, partitions, callSite, listener, properties) => // 接受到JobSubmit事件后执行此方法
